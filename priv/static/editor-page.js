/* generated by scripts/extract_client_js.mjs — re-run extractor; do not hand-fix escapes */
      ...
      const MAX_ATTACH_BYTES = Number(pageMeta.max_attach_bytes) || 50 * 1024 * 1024;
      const MAX_ATTACH_PER_NOTE = Number(pageMeta.max_attach_per_note) || 80;
      ...
      async function postUrl(url) {
              ...
              if ((attachments || []).length >= MAX_ATTACH_PER_NOTE) {
                setStatus("too many attachments (max " + MAX_ATTACH_PER_NOTE + ")");
                return;
              }
            ...
            async function postFiles(fileList) {
              ...
              for (let i = 0; i < files.length; i++) {
                if ((attachments || []).length >= MAX_ATTACH_PER_NOTE) {
                  setStatus("stopped · max " + MAX_ATTACH_PER_NOTE + " attachments");
                  break;
                }
                const file = files[i];
                ...
                if (file.size > MAX_ATTACH_BYTES) {
                  setStatus("too large · " + name + " · max " + fmtSize(MAX_ATTACH_BYTES));
                  uploadBusy = false;
                  renderAtts();
                  return;
                }
                if (file.size === 0) {
                  setStatus("empty file · " + name);
                  continue;
                }
                setStatus(total > 1
                ...
                try {
                  const r = await fetch(BASE + "/api/note/" + slug + "/attachments", {
                    ...
                    headers: {
                      ...
                      // encode so non-ASCII names round-trip; server still sanitizes
                  ...
                  if (!r.ok) {
                    ...
                    if (r.status === 413) detail = detail || (" · max " + fmtSize(MAX_ATTACH_BYTES));
